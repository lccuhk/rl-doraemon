...
def analyze_decision_process(data, output_dir="./logs_high_reward/analysis"):
    ...
    print(f"\n【决策过程分析】")
    print(f"  1. 优先和牌逻辑: 已启用，当有和牌机会时90%概率优先选择和牌")
    print(f"  2. 日志功能: 已启用，当有和牌机会时会自动打印详细日志")
    print(f"  3. 奖励机制: 和牌奖励+15.0，输家惩罚-5.0")
    
    if high_rewards:
        print(f"\n  【重要发现】")
        print(f"    最近出现了{len(high_rewards)}个高奖励回合(>3.0)")
        print(f"    这可能意味着模型开始学习到一些有效的策略")
        print(f"    虽然还没有实际和牌(+15.0)，但中间奖励表现更好")
    
    print(f"\n【可能的和牌失败原因】")
    print(f"  1. 训练时间不足: {status['progress']:.1f}% 完成")
    print(f"  2. 和牌是稀疏奖励: 需要和牌机会出现才能学习")
    print(f"  3. epsilon探索: 初期epsilon高，随机动作可能错过和牌机会")
    print(f"  4. 随机对手: 对手是随机策略，不会主动送和牌")
    
    print(f"\n【建议】")
    print(f"  1. 继续训练到5000回合")
    print(f"  2. 观察高奖励回合是否持续出现")
    ...
    analysis['analysis']['key_findings'] = {
        'high_rewards_count': len(high_rewards) if 'high_rewards' in locals() else 0,
        'dqn_vs_opponents': 'DQN表现优于对手' if evaluations and dqn_rewards[-1] > sum([e['avg_rewards'][i] for i in range(1,4)])/3 else 'DQN表现一般'
    }
    
    output_file = os.path.join(output_dir, f'detailed_analysis_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json')
...
"""
    ...
    <div class="success">
        <strong>重要发现:</strong>
        <br>最近{len(recent_rewards)}回合中有{len(high_rewards)}个高奖励回合(>3.0)
        <br>这表明模型可能开始学习到一些有效的策略！
        <br>高奖励回合: {', '.join([f'回合{i+1}({r:.2f})' for i, r in high_rewards])}
    </div>
"""
...
"""
    ...
    <div class="analysis-section">
        <h2 style="margin-top: 0;">决策过程分析</h2>
        
        <div class="analysis-item">
            <div class="title">1. 优先和牌逻辑</div>
            <div class="content">已启用，当有和牌机会时90%概率优先选择和牌动作。这确保模型能体验到和牌的高奖励(+15.0)。</div>
        </div>
        
        <div class="analysis-item">
            <div class="title">2. 日志功能</div>
            <div class="content">已启用，当有和牌机会时会自动打印详细日志，包括手牌、Q值对比、决策过程等信息。</div>
        </div>
        
        <div class="analysis-item">
            <div class="title">3. 奖励机制</div>
            <div class="content">和牌奖励+15.0(原+1.0，提升15倍)，输家惩罚-5.0(原-0.33，提升15倍)。大幅增加和牌的吸引力。</div>
        </div>
    </div>
    ...
    <div class="analysis-section">
        ...
        <div class="analysis-item">
            ...
            <div class="content">当前仅完成 {status['progress']:.1f}% 的训练，DQN需要大量样本才能学习到和牌策略。建议继续训练到5000回合。</div>
        </div>
    ...
    <div class="analysis-section">
        ...
        <div class="analysis-item">
            ...
            <div class="content">继续训练到5000回合，观察模型是否能学习到和牌策略。</div>
        </div>
...
"""
    ...
    output_file = os.path.join(output_dir, f'detailed_report_{datetime.now().strftime("%Y%m%d_%H%M%S")}.html')
    with open(output_file, 'w', encoding='utf-8') as f:
...
def main():
    ...
    if data['status']:
        analysis = analyze_decision_process(data)
